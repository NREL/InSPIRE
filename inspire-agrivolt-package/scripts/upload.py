...
def main() -> None:
    ...
    parser.add_argument('local-zarr-name', type=str, help='zarr name, ex: 01.zarr') 
    parser.add_argument('final-dir', type=str, help='directory containing final merged zarrs as generated by combine step.')

    args = parser.parse_args()

    zarr = args.local_zarr_name
    final_dir = Path(args.final_dir)

    if not final_dir.exists():
        raise FileNotFoundError(f"final-dir not found at path: {str(final_dir)}")

    print(f"using local zarr: {zarr}")

    INSPIRE_PREFIX = f"inspire/{zarr}/"
    LOCAL_ROOT = final_dir / zarr
