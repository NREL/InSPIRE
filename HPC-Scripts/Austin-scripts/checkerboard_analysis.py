...
df1 = pd.read_csv('Results_Temple.csv')
data_GHI = pd.read_csv('metdata_temple.csv')
...
# yground_index = []
# zground_index = []
rows = np.array(range(0,101))
cols = np.array(range(0,101))
GHI_vals = pd.DataFrame(index = rows, columns = cols)
GHI_norm_vals = pd.DataFrame(index = rows, columns = cols)
GHI_vals_transposed = GHI_vals.T
GHI_norm_transposed = GHI_norm_vals.T

positions = np.array(range(0,101))
...
fig = px.imshow(GHI_norm_transposed, origin="lower", aspect='equal')
...
fig = px.imshow(GHI_vals.T, origin="lower", aspect='equal')
