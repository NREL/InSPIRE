                ...
                if pos.size:
                    ...
                    def _to_py(v):
                        try:
                            ...
                            return v
                    top_list = [(_to_py(coord_vals[pos[i]]), int(counts_pos[i])) for i in top_sel]
...
def gids_with_time_nans(
    ...
    for var in vars_to_check:
        ...
        var_bad_gids = da.isnull().any(dim=(time_name,) + other_dims) if other_dims else da.isnull().any(dim=time_name)
    ...
    if return_details:
        ...
        per_var_counts = {v: int(m.sum().compute().item()) for v, m in per_var_masks.items()}
