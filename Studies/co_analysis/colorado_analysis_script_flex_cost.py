...
def run_analysis(i):
    ...
    ppa_prices =  [x / 2000.0 for x in range(88, 141, 1)]
    #ppa_prices = [0.051]
    crop_profit = range(min_profit, max_profit + 1, 50 * acres)

    prices =  [x / 100.0 for x in range(138, 160, 1)] #1.30 to #1.80
    ...
    columns = ["County", "Lat", "Long", "Config", "Crop", "Solar capacity", "CAPEX", "PPA price", "Open Air Crop Profit", "Specific Crop Profit", \
                "NPV", "IRR", "Capacity Factor", "Annual Energy Production (year 1)", "Energy Value (year 1)", "Discounted Payback", "$/W"]
        ...
        for price in ppa_prices:
            for profits in crop_profit:
                for crop in crops:
                    ...
                    so.SystemCosts.total_installed_cost = cost * 1000 * pv.SystemDesign.system_capacity
...
if __name__ == "__main__":
    ...
    results_df.to_csv("adams_co_price_comparison_2_config.csv")