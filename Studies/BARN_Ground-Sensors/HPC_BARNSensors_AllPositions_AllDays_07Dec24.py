...
def simulate_single(weatherfile, startdate, results_path, smaller_sim):
    ...
    singlesim_path = os.path.join(results_path,simpath)
    #Check if simulation has already completed
    print("Current path", singlesim_path)

    if not os.path.exists(singlesim_path):
        os.makedirs(singlesim_path, exist_ok=True)

    print("path", singlesim_path)
    time.sleep(2)
    radObj = br.RadianceObj(simpath,singlesim_path)
...
if __name__ == "__main__":
    ...
    # NOTE FROM SILVANA 07Dec24: the HPC would get confused with folders if doing a lot of days; tested only 10 at a time and that worked.
    if smaller_sim:
        start = datetime.datetime(2023, 9, 26, 0, 0)
        end = datetime.datetime(2023, 9, 26, 0, 0)
    else:
        start = datetime.datetime(2023, 10, 21, 0, 0)
        end = datetime.datetime(2023, 10, 31, 0, 0)
    ...
    kestrel = {
        'manager': 'slurm',
        'n_jobs': 1,  # Number of nodes used for parallel processing #1. Max 2 for debug queue.
        'cores': 104, #This is the total number of threads in all workers was #26
        'memory': '256GB',
        'account': 'inspire',
        # 'queue': 'standard', #'debug'
        'queue': 'debug', #'debug'
        'walltime': '0:40:00', # The time at which the run will be abandoned.
    ...
    results_path = "/scratch/sayala/chongseok"+"_"+now.strftime('%Y-%m-%d_%Hh%M') # scratch folder is at the same level as "home," and everyone has one.
    ...
    weatherfile = r'/home/sayala/WeatherFiles/PSM3_60T_withMinutes.csv'
    #weatherfile = r'/home/sayala/WeatherFiles/PSM3_60T.csv'
